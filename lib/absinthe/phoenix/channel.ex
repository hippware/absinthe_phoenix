defmodule Absinthe.Phoenix.Channel do
  ...
  defp run(document, schema, pipeline, options) do
    ...
    case Absinthe.Pipeline.run(document, apply(module, fun, [schema, options])) do
      {:ok, %{result: %{continuation: continuation} = result, execution: exec}, _phases} ->
        {:more, Map.delete(result, :continuation), continuation, exec.context}
      {:ok, %{result: result, execution: exec}, _phases} ->
        {:ok, result, exec.context}
      {:error, msg, _phases} ->
        {:error, msg}
    end
  end
